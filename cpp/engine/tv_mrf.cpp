...
// 論文 3.1: TV-MRF 更新則 (Split Bregman法)
void DenoiseEngine::tv_mrf(const TVMRFParams& p_in, std::function<void(const IterationResult&)> on_step) {
    TVMRFParams p = p_in;
    double mu = p.alpha;
    ...
    std::vector<double> x_vec = centered_noisy; // 作業用MAP解 (centered domain)
    ...
    if (!p.is_learning) {
        for (int iter = 1; iter <= 100; ++iter) {
            ...
            double diff = 0;
            for (int i = 0; i < n; ++i) diff += std::abs(x_vec[i] - x_old[i]);
            if ((diff / n) < conv_epsilon) break;
        }
        report_progress(p.max_iter, 0.0, x_vec, y_ave, "CONVERGED", on_step);
        return;
    }
    ...
    for (int iter = 1; iter <= p.max_iter; ++iter) {
        std::vector<double> x_old = x_vec;
        // 1. x-step Update
        report_progress(iter, 0.0, x_vec, y_ave, "MAP OPTIMIZATION", on_step);
        for (int step = 0; step < 2; ++step) {
            for (int y = 0; y < h; ++y) {
                for (int x = 0; x < w; ++x) {
                    int i = get_idx(x, y);
        ...
        // 2. d-step Shrinkage
        report_progress(iter, 0.0, x_vec, y_ave, "TV REGULARIZATION", on_step);
        for (int y = 0; y < h; ++y) {
            for (int x = 0; x < w; ++x) {
                int i = get_idx(x, y);
                if (x < w - 1) {
        ...
        // 3. b-step Update
        report_progress(iter, 0.0, x_vec, y_ave, "BREGMAN UPDATE", on_step);
        for (int i = 0; i < n; ++i) {
            int x = i % w, y = i / w;
            if (x < w - 1) b_x[i] += (x_vec[i] - x_vec[get_idx(x+1, y)] - d_x[i]);
            if (y < h - 1) b_y[i] += (x_vec[i] - x_vec[get_idx(x, y+1)] - d_y[i]);
        ...
        if (iter % 10 == 0 || iter == p.max_iter || (mae / n) < conv_epsilon) {
            report_progress(iter, 0.0, x_vec, y_ave, "STABLE", on_step);
